...
export const fetchReds = createAsyncThunk("reds/fetchReds", async () => {
...
export const selectRedById = (id) => (state) => {
  return state.reds.redsArray.find((red) => red.id === parseInt(id));
};

export const selectFeaturedRed = (state) => {
  return {
    featuredItem: state.reds.redsArray.find((red) => red.featured),
    isLoading: state.reds.isLoading,
    errMsg: state.reds.errMsg,
  };
};
